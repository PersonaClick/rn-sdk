 ...
 */
export const getLastPushTokenSentDate = async () => {
  try {
    const savedDate = await AsyncStorage.getItem(SDK_STORAGE_NAME + '_push_token_last_sent_date');
 ...
 */
export const saveLastPushTokenSentDate = async (date) => {
  try {
    if (!(date instanceof Date) || isNaN(date)) {
      throw new TypeError('The provided argument is not a valid Date object.');
    }
    await AsyncStorage.setItem(SDK_STORAGE_NAME + '_push_token_last_sent_date', date.toISOString());
  } catch (error) {
    return error;
  }
}
