...
class PopupLogic {
  ...
  static async prepare(sdkInstance, popupData, manual = false, showCallback = null) {
    ...
    if (showCallback && typeof showCallback === 'function') {
      // Apply delay before showing. Pass the triggering sdk through so the overlay renders and
      // tracks against the shop that owns this popup — not whichever instance registered last.
      const delayMs = (popupData.delay || 0) * 1000
      setTimeout(() => {
        showCallback(popupData, sdkInstance)
      }, delayMs)
    } else {
